/**
 * Scheduled job (scheduledAutoEndPastConsultations): moves a consultation into history only when BOTH are true:
 *
 * 1. Slot end time has passed (now >= computed slot end using isoOffset, e.g. +08:00).
 * 2. Nobody is in the video room: no truthy entries in videoCall/room.participants, or room missing / already ended.
 *
 * Then: appointment status → completed, videoSessionEndedAt + consultationNotesAutoFinalizedAt set,
 * videoCall/room → status ended (endedBy: schedule), vet schedule slot → completed.
 * If anyone is still in the room past end time, this job skips that appointment until the room is empty.
 */
'use strict';
...
module.exports = { runAutoEndPastConsultations };
